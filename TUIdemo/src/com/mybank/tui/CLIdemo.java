...
/**
 * Console client for 'Banking' example (Lab 8 - Fixed Warnings)
 *
...
public class CLIdemo {
        ...
        while ((line = readLine(reader, "")) != null) {
            if ("help".equals(line)) {
                ...
                System.out.println(ANSI_RED 
                    + "Invalid command. For assistance press TAB or type \"help\" then hit ENTER." 
                    + ANSI_RESET);
            }
        }
    ...
    private void printCustomerList() {
        ...
        if (Bank.getNumberOfCustomers() > 0) {
            System.out.println("\nLast name\tFirst Name\tBalance (First Acct)");
            System.out.println("---------------------------------------");
            for (int i = 0; i < Bank.getNumberOfCustomers(); i++) {
                Customer c = Bank.getCustomer(i);
                System.out.printf("%s\t\t%s\t\t$%.2f%n", 
                    c.getLastName(), c.getFirstName(), c.getAccount(0).getBalance());
            }
        } else {
    ...
    private void printCustomerDetails(String line) {
        try {
            ...
            // Статична перевірка без створення/виклику локальної змінної bank
            ...
            for (int i = 0; i < cust.getNumberOfAccounts(); i++) {
                String accType = cust.getAccount(i) instanceof CheckingAccount ? "Checking" : "Savings";
                System.out.printf("%s\t\t%s\t\t%s\t$" + "%.2f%n", 
                    cust.getLastName(), cust.getFirstName(), accType, cust.getAccount(i).getBalance());
            }
        } catch (Exception e) {
            System.out.println(ANSI_RED + "ERROR! Wrong customer number!" + ANSI_RESET);
        }
    ...
    private void printReport() {
        AttributedStringBuilder a = new AttributedStringBuilder()
            .append("\nCUSTOMER REPORT", AttributedStyle.BOLD.foreground(AttributedStyle.CYAN));
        System.out.println(a.toAnsi());
        System.out.println("=".repeat(55));
        System.out.printf("%-5s %-15s %-12s %10s%n", "No.", "Name", "Acct Type", "Balance");
        System.out.println("-".repeat(55));

        // Виправлено виклики методів Bank на статичні у циклах звіту
        for (int i = 0; i < Bank.getNumberOfCustomers(); i++) {
